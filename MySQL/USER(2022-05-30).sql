...
-- MySQL 은 DB 와 관련된 명령을 실행할 때
-- 가장 먼저 어떤 DB 를 사용할 것인지 알려줘야 한다
USE mydb;
DROP TABLE tbl_test;
-- Table 을 생성할 때 칼럼의 이름은 사전에 있는 키워드는 가급적
-- 사용하지 않는 것이 좋다
...
대부분의 DBMS 들은 먼저 Database 를 생성하고 
사용자가 Database 에 접근할 수 있는 권한을 부여하고
...
데이터를 INSERT 할 때마다 자동으로 Sequence 값이 생성되어
추가된다
5.x MySQL 에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
5.x 숫자 type 은 주로 INT 형을 사용하는데 INT 형은 정수 11자리이다
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT 가 불가능해지는 문제가 있었다.
5.7 버전 이후에는 BIGINT 형 타입이 도입되어 문제를 해결하였다.
DB table 을 만들 때 AUTO_INCREMENT 칼럼에는 반드시 BIGINT 형을 지정하자.

MySQL 숫자형 데이터 타입
TINYINT : 1 Byte : -128 ~ 127 까지만 저장 가능
SMALLINT : 2 Byte : -32768 ~ 23762 까지
MEDIUMINT : 3 Byte : 0 ~ 16777215, - 8388608 ~ 8388607
INT : 4 Byte : -217483648 ~ 214783647
BIGINT : 8 Byte : -2 의 7승 -1 부터 2의 7승까지
*/
...
PK 아닌 경우는 반드시 NOT NULL 과 UNIQUE 를 같이 설정한다.
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','이몽룡','컴공과',3,'010-111-1111','익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('111','홍길동','컴공과',1,'010-6396-27850','익산시');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S00','홍길동','컴공과',1,'010-1122','익산시');
