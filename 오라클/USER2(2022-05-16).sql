...
/*
tbl_score 데이터는 한 학생의 과목점수가 각각 레코드별로 저장되어있다.
과목 수가 늘어나면 학번, 과목명, 점수 형식으로 계속 저장해주면 된다.

실제 필요한 데이터는 학번, 과목명, 점수 칼럼인데, PK 를 설정하기 위해 SEQ 라는
칼럼이 추가된 상태
SEQ 칼럼이 없이 PK를 설정하려면 학번 + 과목명 으로 두 개의 칼럼을 묶어서
PK 로 해야 한다.
...
WHERE sc_num = '1111' AND sc-subject = '국어'

DELETE FROM tbl_student
WHERE sc_num = '1111' AND sc-subject = '국어'

*/
-- 학번 S0044 인 학생의 미술 점수를 50점에서 80점으로 바꾸고 싶다
-- Update 나 Delete 실행할 때 조건절(WHERE)에 복수의 칼럼 조건을 부여하는 것은
-- 썩 좋지 않다.
...
-- 3. tbl_score.sc_stnum 데이터를 가지고 tbl_student.st_sum 를 SELECT 한 후
