...
INSERT INTO tbl_student(st_num, st_name, st_grade) VALUES ('20220002', '이몽룡', 2);
INSERT INTO tbl_student(st_num, st_name, st_grade) VALUES ('20220003', '성춘향', 3);
...
데이터는 insert 할 때 자동으로 칼럼 데이터를 추가한다.
...
-- seq- belong SEQUENCE 의 NEXTVAL 값을 조회(SELECT) 하라
...
SELECT st_num, st_name, st_grade, b_dcode,  b_stnum, d_name, d_code
FROM tbl_student;
...
SELECT st_num, st_name, b_dcode, d_name, st_grade
FROM tbl_student
...
-- 다음의 SQL 의 결과에서 결과가 한 개도 없어야 참조 무결성 관계가 성립된다.
...
-- child key found
-- FK 를 설정할 대
-- parent table 의 데이터를 삭제하려고 할 때 child table 에 데이터가 있으면
-- 보통 삭제 금지,
-- parent table 데이터를 변경(Update) 하려고 할 때 child table 에 데이터가 있으면
-- 보통 변경 금지
