...
-- 드디어 일주일간 야근이다
-- 경고 : UPDATE 와 DELETE 명령은 WHERE 이 없을 때 절대 실행 금지~~!!
UPDATE tbl_score
SET sc_sbcode = '001';
...
-- 새로 생성한 과목코드(sc_sbcode) 칼럼은 값이 비어있으면 곤란 상황이 발생할 수 있다.
...
-- DBMS 에서는 길이가 같은 문자열은 부등호 조건을 결과를 조회할 수 있다
SELECT *
FROM tbl_score
...
-- 참조 table 에서 ON 조건에 맞는데이터를 조회한다.
...
-- 다수의 table 에 저장된 데이터의 개수를 한 번에 확인하기
